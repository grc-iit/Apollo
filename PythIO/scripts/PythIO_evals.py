...
def plot_prediction(model, testX, y, cut=-1, plot=True ,st='-', prefix=""):
  ...
  mae = metrics.mean_absolute_error(y, pre)

  print(f'{prefix} r2: {r2}, mae: {mae}, mse: {mse}, cost: {executionTime}')
  if plot:
...
def load_all_models():
    ...
    models = {}
    for i in model_list:
      models[i] = load_model(model_dir+i)
    return models


def inference_(models=None, data_path = None, col = 'tps_', window=5, lim=-1, plot_=False, terval=2, prefix=""):
  print(col)
  if models == None:
    model_dir = '../models/'
    model_list = os.listdir(model_dir)
...
#  Add more as needed
  ...
  for k, v in models.items():
    if lim == -1:
      tf.profiler.experimental.start(f'{data_path.split("/")[-1]}-{k}-{col}', tf.profiler.experimental.ProfilerOptions(host_tracer_level=2))
    for i in range(terval):
      plot_prediction(v, testX, y_test, plot = False, prefix=f'{k}-{col}-{prefix}')
    if lim == -1:
...
def main_():
  cols = ['tps_', 'rd_sec/s_', 'wr_sec/s_', 'Avgrq-sz_', 'await_', 'svctm_', '%util_']
  data_paths = ['../datasets/ssd.csv', '../datasets/nvme.csv', '../datasets/hdd.csv']
  win_max = 10
  w = 5
  #  for w in range(6, 10):
  print(f'Window : {w}')
  ...
  for j in data_paths:
    for i in cols:
      ...
      inference_(models, j, window=w, col=i, prefix=f'{i}-{j.split("/")[-1]}-r2-mse ')
      #  print("inference cost")
      inference_(models, j, window=w, lim=6, prefix=f'{i}-{j.split("/")[-1]}-inference ')
  ...
  #      models[j] = train_dataset(le_vals, w, j, ver = 0)
#
