...
def inference_(models=None, data_path = None, col = 'tps_', window=5, lim=-1, plot_=False, terval=5):
  print(col)
  if models == None:
    all_ = load_model('../models/all-model.model')
    all_misc_ = load_model('../models/all-misc.model')

  else :
    all_ = models['all_model']
    all_misc_ = models['all_misc']
    sel_model = models['sel_model']
    sel_misc = models['sel_misc']
...
def main_():
  cols = ['tps_', 'rd_sec/s_', 'wr_sec/s_', 'Avgrq-sz_', 'Avgqu-sz_', 'await_', 'svctm_', '%util_']
  data_paths = ['/content/drive/My Drive/ssd_cleaned_headers.csv']
  win_max = 10
  for w in range(5, 10):
    print(f'Window : {w}')
    ...
    for i in cols:
      for j in data_paths:
        inference_(models, j, col=i)
        print('---------------- single prediction cost -----------------')
        inference_(models, j, lim=6)
  ...
  for w in range(5, win_max):
    for i in data_paths:
      for j in cols:
        ...
        models[j] = train_dataset(le_vals, w, j, ver = 1)
