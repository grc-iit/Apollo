...
def get_benchmark():
    ...
    rand_command = ior_path+'ior -a=POSIX -O summaryFormat=JSON'
    seq_command = ior_path+'ior -a=POSIX -z -O summaryFile=random.json'
    sequential = json.loads(subprocess.check_output(rand_command, shell=True))
    random = json.loads(subprocess.check_output(seq_command, shell=True))
