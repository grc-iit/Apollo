...
def get_benchmark():
    rand_command = 'ior -a=POSIX -O summaryFormat=JSON'
    seq_command = 'ior -a=POSIX -z -O summaryFile=random.json'
    sequential = json.loads(subprocess.check_output(ior_path+rand_command, shell=True).decode('utf-8'))
    random = json.loads(subprocess.check_output(ior_path+seq_command, shell=True).decode('utf-8'))
